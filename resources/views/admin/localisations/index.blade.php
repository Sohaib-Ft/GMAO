...
@section('header', ' Localisataion des Équipements')
...
<div class="space-y-6" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <form action="{{ route('localisations.index') }}" method="GET" id="filterForm" class="flex flex-1 items-center max-w-lg w-full">
        ...
        <button @click="openCreate()" class="group flex items-center px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-200 font-medium w-full md:w-auto">
            <i class='bx bx-plus-circle mr-2 text-xl group-hover:rotate-90 transition-transform'></i> Ajouter un local
        </button>
    </div>
    ...
    <!-- Table -->
    <div id="localisations-table-container" class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-widest">Nom du local</th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-widest">Équipements liés</th>
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <span class="font-semibold text-gray-700">{{ $loc->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs font-bold">
                                {{ $loc->equipements_count }} équipement(s)
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <button @click="openEdit({{ $loc->id }}, '{{ addslashes($loc->name) }}')" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Modifier">
                                <i class='bx bx-edit-alt text-xl'></i>
                            </button>
                            <button type="button" onclick="openDeleteModal('{{ route('localisations.destroy', $loc) }}', '{{ addslashes($loc->name) }}')"
                                    class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition"
                                    {{ $loc->equipements_count > 0 ? 'disabled title=Lié_à_des_équipements' : 'title=Supprimer' }}>
                                <i class='bx bx-trash text-xl {{ $loc->equipements_count > 0 ? 'opacity-30' : '' }}'></i>
                            </button>
                        </td>
                    </tr>
                    @empty
            ...
            <div x-show="showModal"
                ...
                <div>
                    <div class="flex items-center mb-6">
                        <div class="w-12 h-12 bg-blue-100 rounded-2xl flex items-center justify-center text-blue-600 mr-4 text-2xl">
                            <i class='bx bx-buildings'></i>
                        </div>
                        <div>
                            <h3 class="text-xl font-bold text-gray-900" x-text="isEdit ? 'Modifier le local' : 'Nouveau local'"></h3>
                            <p class="text-sm text-gray-500">Entrez le nom de l'emplacement physique.</p>
                        </div>
                    </div>

                    <form :action="isEdit ? '/admin/localisations/' + currentId : '{{ route('localisations.store') }}'" method="POST">
                        @csrf
                        ...
                        <div class="mb-6">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nom du Site</label>
                            <input type="text" name="name" x-model="currentName" required
                                   class="w-full px-4 py-3 bg-gray-50 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                                   placeholder="Ex: Etage 1, Entrepôt Nord...">
                        </div>

                        <div class="flex flex-col sm:flex-row gap-3">
                            <button type="submit" class="flex-1 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-200" x-text="isEdit ? 'Mettre à jour' : 'Enregistrer'"></button>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Êtes-vous sûr de vouloir supprimer <span id="itemNameToDelete" class="font-bold text-gray-800">cette localisation</span> ?
                                </p>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const container = document.getElementById('localisations-table-container');
    ...
    function updateResults() {
        ...
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(html => {
                ...
                const newContent = doc.getElementById('localisations-table-container');
